...
const checkbox = renderer.create('<div class="form-check"></div>', function (
  $node,
  options
) {
  $node.html(
...
const ui = function (editorOptions) {
  return {
    ...
    palette($node, options) {
      return renderer.create('<div class="note-color-palette"/>', function (
        $node,
        options
      ) {
        const contents = []
        for (
          let row = 0, rowSize = options.colors.length;
          row < rowSize;
          row++
        ) {
