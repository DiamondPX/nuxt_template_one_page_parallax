...
class WrappedRange {
  ...
  nativeRange() {
    if (env.isW3CRangeSupport) {
      ...
      w3cRange.setEnd(
        this.ec,
        this.sc.data ? Math.min(this.eo, this.sc.data.length) : this.eo
      )
  ...
  getWordsRange(findAfter) {
    ...
    const isNotTextPoint = function (point) {
...
export default {
  ...
  createFromBodyElement(bodyElement, isCollapseToStart = false) {
  ...
  createFromSelection() {
    ...
    if (env.isW3CRangeSupport) {
      ...
      const endPoint = textRangeToPoint(textRangeEnd, false)
   ...
   * @return {WrappedRange}
   */
  createFromBookmark(editable, bookmark) {
   ...
   * @return {WrappedRange}
   */
  createFromParaBookmark(bookmark, paras) {
    const so = bookmark.s.offset
    const eo = bookmark.e.offset
