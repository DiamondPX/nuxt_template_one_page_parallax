...
export default class Buttons {
  ...
  isFontDeservedToAdd(name) {
    ...
    return (
      ...
      !env.genericFontFamilies.includes(name)
    )
  }
  ...
  addToolbarButtons() {
          ...
          this.ui.dropdown({
            ...
            template: (item) => {
              if (typeof item === 'string') {
                item = {
                  tag: item,
                  title: this.lang.style.hasOwnProperty(item)
                    ? this.lang.style[item]
                    : item,
                }
              }
    ...
    for (
      let styleIdx = 0, styleLen = this.options.styleTags.length;
      styleIdx < styleLen;
      styleIdx++
    ) {
      ...
      this.context.memo('button.style.' + item, () => {
        return this.button({
          ...
          contents:
            '<div data-value="' + item + '">' + item.toUpperCase() + '</div>',
    ...
    this.context.memo('button.underline', () => {
      return this.button({
        ...
        click: this.context.createInvokeHandlerAndUpdateState(
          'editor.underline'
        ),
      }).render()
    })
      ...
      if (this.options.addDefaultFonts) {
        ...
        $.each(styleInfo['font-family'].split(','), (idx, fontname) => {
          ...
          if (this.isFontDeservedToAdd(fontname)) {
            if (!this.options.fontNames.includes(fontname)) {
      ...
      return this.ui
        .buttonGroup([
          ...
          this.ui.dropdownCheck({
            ...
            items: this.options.fontNames.filter(
      ...
      return this.ui
        .buttonGroup([
          this.button({
            className: 'dropdown-toggle',
            contents: this.ui.dropdownButtonContents(
              '<span class="note-current-fontsizeunit"/>',
              this.options
            ),
    ...
    this.context.memo('button.ul', () => {
      return this.button({
        ...
        tooltip:
          this.lang.lists.unordered +
          this.representShortcut('insertUnorderedList'),
      ...
      return this.ui
        .buttonGroup([
          this.button({
            className: 'dropdown-toggle',
            contents: this.ui.dropdownButtonContents(
              this.ui.icon(this.options.icons.textHeight),
              this.options
            ),
        ...
        .buttonGroup(
          [
            this.button({
              className: 'dropdown-toggle',
              contents: this.ui.dropdownButtonContents(
                this.ui.icon(this.options.icons.table),
                this.options
              ),
          ...
          ],
          {
            callback: ($node) => {
              ...
              $catcher
                ...
                .mousedown(
                  this.context.createInvokeHandler('editor.insertTable')
                )
  ...
  build($container, groups) {
    for (
      let groupIdx = 0, groupLen = groups.length;
      groupIdx < groupLen;
      groupIdx++
    ) {
