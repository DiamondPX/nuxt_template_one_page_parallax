  ...
  function ($node, options) {
    const markup = Array.isArray(options.items)
      ? options.items.map(function (item) {
          ...
          $temp
            .html([icon(options.checkClassName), ' ', content])
            .data('item', item)
          return $temp
        })
      : options.items
...
const palette = renderer.create('<div class="note-color-palette"/>', function (
  $node,
  options
) {
  const contents = []
...
const checkbox = renderer.create('<div class="checkbox"></div>', function (
  $node,
  options
) {
  $node.html(
...
const ui = function (editorOptions) {
  return {
    ...
    onDialogShown($dialog, handler) {
      $dialog.one('note.modal.show', handler)
    },

    onDialogHidden($dialog, handler) {
     ...
     * @returns {*}
     */
    getDialogBody($dialog) {
      return $dialog.find('.note-modal-body')
    },

    createLayout($note) {
      const $editor = (editorOptions.airMode
